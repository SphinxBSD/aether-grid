...
/**
 * Generate TypeScript bindings for contracts for local network
 *
 * Generates type-safe client bindings from locally deployed contracts
 */
...
Examples:
  ...
  bun run bindings:local number-guess
  bun run bindings:local twenty-one number-guess
`);
}

console.log("📦 Generating TypeScript bindings for local network...\n");
...
if (existsSync("local-deployment.json")) {
  const deploymentInfo = await Bun.file("local-deployment.json").json();
...
if (missing.length > 0) {
  console.error("❌ Error: Missing contract IDs (need either local-deployment.json or .env):");
...
for (const contract of contractsToBind) {
  ...
  console.log(`Generating bindings for ${contract.packageName} (local network)...`);
  try {
...
console.log("🎉 Local bindings generated successfully!");
console.log("\nGenerated files:");
for (const contract of contractsToBind) {
