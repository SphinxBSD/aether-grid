...
// Unit tests for the aether-grid contract using a simple mock GameHub.
...
use crate::{Error, AetherGridContract, AetherGridContractClient};
...
fn setup_test() -> (
    Env,
    AetherGridContractClient<'static>,
    ...
    // Deploy aether-grid with admin and GameHub address
    let contract_id = env.register(AetherGridContract, (&admin, &hub_addr));
    let client = AetherGridContractClient::new(&env, &contract_id);

    // Register aether-grid as a whitelisted game (mock does nothing)
